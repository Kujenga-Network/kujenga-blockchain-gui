...
global.daemon_rpc_ws = `wss://${self_hostname}:55324`;
...
function loadConfig(net) {
  try {
    // check if KUJENGA_ROOT is set. it overrides 'net'
    const config_root_dir =
      'KUJENGA_ROOT' in process.env
        ? process.env.KUJENGA_ROOT
        : path.join(os.homedir(), '.kujenga', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 55324); // jshint ignore:line
